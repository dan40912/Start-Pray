...
| 首頁極簡錄音入口（新） | `/`（`HomePrayerHero.js` 顯示真實 Prayer → `PrayerRecorder`） | 首頁載入一件真實 `HomePrayerCard`（`sort=needsPrayer`），點擊 CTA→同頁切換成錄音流程，錄音送出後建立 `PrayerResponse` 附掛於該 Prayer | `HomePrayerHero.js`、`src/components/prayer-recorder/*`、`HomeLandingPage.js` | `POST /api/responses`（Real API tested，見 [[10-Implementation-Plan]] Commit A） | `PrayerResponse`（`homeCardId` 綁定首頁顯示的那張卡片，非隨機） | 瀏覽器 MediaRecorder | **2026-08-04 更新**：不再隨機挑卡，改為與首頁顯示內容一致；左右滑動與陪伴模式為後續 Commit 範圍 |
